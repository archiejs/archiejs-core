        ...
        config.forEach(function (plugin, index) {
            ...
            if (!plugin.hasOwnProperty("setup")) {
                ...
                try {
                    ...
                    debug(`main file FOUND for ${plugin.packagePath}`);
                } catch (e) {
                    // its okay if we did not find the module (maybe different pattern was
                    // followed than having a main file - index.js)
                    if (e.code !== 'MODULE_NOT_FOUND') { // all non-module find errors
                        throw (e);
                    } else if (e.message.match(plugin.packagePath) === null) { // some other package generated this error - maybe some npm module is missing
                        throw (e);
                    } else {
                        debug(`main file NOT found/specified in ${plugin.packagePath}`);
                    }
                }
            }
...
// callback
Archie.prototype.loadPlugins = async function (config) {
    var app = this;
    ...
    for (var idx in sortedConfig) {
        var plugin = sortedConfig[idx]
 ...
 * Register a plugin 
 */
Archie.prototype.registerPlugin = async function (plugin) {
    var app = this;
    ...
    var pluginServices = await plugin.packageEnhancer.setupPlugin.call(plugin.packageEnhancer, plugin, imports)
