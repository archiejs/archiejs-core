...
function Archie(config, callback) {
    var app = this;
    app.config = [];
    app.destructors = [];
    app.services = {
...
Archie.prototype.loadPlugins = function(config) {
    ...
    var pluginRunner = Promise.resolve();
    sortedConfig.forEach(function(plugin) {
      pluginRunner.then(app.registerPlugin(plugin));
    });
    return pluginRunner;
};
 ...
 * Register a plugin 
 */
Archie.prototype.registerPlugin = function(plugin) {
  var app = this;
  ...
  // call the setup function in enhancer, which further calls
  // register function (below)
  return Promise.resolve(plugin, imports)
    .then(plugin.packageEnhancer.setupPlugin.bind(plugin.packageEnhancer)) // calls setupPlugin(plugin, imports) in right context
    .then((registerServices) => {
      // check if all services are provided
      for(var name of plugin.provides) {
        // register services
        if (!registerServices.hasOwnProperty(name)) {
          var err = new Error("Plugin failed to provide " + name + " service.");
          throw (err);
          return;
        }

        // copy provided serviceis (via plugin) to app.services
        services[name] = registerServices[name];

        // check for user mistake - forgot module.exports.
        if( isEmptyService(services[name]) ){
      ...
      if (registerServices && registerServices.hasOwnProperty("onDestroy")) {
        app.destructors.push(registerServices.onDestroy);
      }
      ...
      plugin.destroy = function() {
        ...
        if (registerServices && registerServices.hasOwnProperty("onDestroy")) {
          app.destructors.splice(app.destructors.indexOf(registerServices.onDestroy), 1);
          registerServices.onDestroy();
        }
...
function isEmptyService(obj){
    // service encapsulates objects
    for(var prop in obj){
        return false;
    }
