...
var BaseEnhancerClass = exports.BaseEnhancerClass = require('./base-enhancer-class');
...
(function () {
    ...
    function resolveConfig(config, base) {
        ...
        config.forEach(function (plugin, index) {
            ...
            if (plugin.hasOwnProperty("packagePath")) {
                ...
                var msgs = [];
                if (!plugin.provides) msgs.push("has no provide statement");
                if (!plugin.consumes) msgs.push("has no consume statement");
                if (msgs.length > 0) {
                  console.warn("About module : " + plugin.packagePath);
                  console.warn(msgs);
                }
            ...
            if(!plugin.hasOwnProperty("setup")) {
                ...
                try{
                    ...
                    if (e.code === 'MODULE_NOT_FOUND') {
                      // if the setup function is not found -
                      // NOTE: setup function is optional here and base enhancer
                      // also provides setup in some cases
                      console.warn("No setup loaded from " + plugin.packagePath);
                    } else {
    ...
    function resolveModule(base, modulePath) {
       ...
       if (packagePath) {
        ...
        var provides = metadata.provides || module.provides;
        var consumes = metadata.consumes || module.consumes;
...
Archie.prototype.registerPlugin = function(plugin, next) {
    ...
    try {
        // this should be called from the enhancer
        plugin.packageEnhancer.setupPlugin(plugin, imports, register);
    } catch(e) {
