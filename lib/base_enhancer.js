              ...
              plugin.provides.filter(key => {
                  ...
                  if ( typeof plugin.enhancers[key] === 'string' ) {  // leaf
                     ...
                     if (registerClassLogic) {
                        ...
                        let inst = new ConstructorFn (plugin, imports);

                        // what if the constructor returns a promise 
                        if (inst instanceof Promise) {
                           debug(`is promise`);
                           return inst.then((val) => {
                             debug(`registered ${key}`);
                             _registerObjs[key] = val;
                           });
                        } else {
                          _registerObjs[key] = inst;
                          return false;
                        }
